...
function fcmanager_options_sanitize_callback($input)
{
    foreach (['player', 'volunteer', 'referee'] as $type) {
        $input["fcmanager_{$type}_publish_birthday_by_default"] = isset($input["fcmanager_{$type}_publish_birthday_by_default"]) && $input["fcmanager_{$type}_publish_birthday_by_default"] == 1 ? 1 : 0;
        $input["fcmanager_{$type}_publish_age_by_default"] = isset($input["fcmanager_{$type}_publish_age_by_default"]) && $input["fcmanager_{$type}_publish_age_by_default"] == 1 ? 1 : 0;
    }
...
function fcmanager_settings_init()
{
    ...
    foreach (
        [
            'player' => __('Player Settings', 'football-club-manager'),
            'volunteer' => __('Volunteer Settings', 'football-club-manager'),
            'referee' => __('Referee Settings', 'football-club-manager')
        ] as $type => $label
    ) {
        // Register settings section for each type.
        add_settings_section(
            "fcmanager_section_{$type}_settings",
            sprintf($label, ucfirst($type)),
            null,
            'fcmanager'
        );

        // Register "Publish birthday by default" toggle for each type.
        add_settings_field(
            "fcmanager_{$type}_publish_birthday_by_default",
            __('Publish birthday by default', 'football-club-manager'),
            'fcmanager_field_toggle_callback',
            'fcmanager',
            "fcmanager_section_{$type}_settings",
            array('label_for' => "fcmanager_{$type}_publish_birthday_by_default")
        );

        // Register "Publish age by default" toggle for each type.
        add_settings_field(
            "fcmanager_{$type}_publish_age_by_default",
            __('Publish age by default', 'football-club-manager'),
            'fcmanager_field_toggle_callback',
            'fcmanager',
            "fcmanager_section_{$type}_settings",
            array('label_for' => "fcmanager_{$type}_publish_age_by_default")
        );
    }
