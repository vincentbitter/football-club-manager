<?php
/*
 * Plugin Name: Football Club Manager
 * Plugin URI: https://github.com/vincentbitter/football-club-manager
 * Description: Easily create a website for your football club to publish teams, players and fixtures.
 * Version: 0.1.0
 * Requires at least: 6.8.1
 * Requires PHP: 8.2
 * Author: Vincent Bitter
 * Author URI: https://vincentbitter.nl
 * License: GPLv3
 * License URI: http://www.gnu.org/licenses/gpl-3.0.html
 * Text Domain: football-club-manager
 * Domain Path: /languages
 */

if (! defined('ABSPATH')) {
    exit; // Exit if accessed directly.
}

// Register custom post types
require_once('includes/post-types/team.php');
require_once('includes/post-types/player.php');
require_once('includes/post-types/match.php');

// Register administration pages
require_once('admin/page_dashboard.php');

// Register administration menu
if (! function_exists('fcm_register_administration_menu')) {
    function fcm_register_administration_menu()
    {
        add_menu_page(
            'Football Club Manager',
            'Football Club Manager',
            'edit_posts',
            'fcm',
            null,
            'data:image/svg+xml;base64,',
            50
        );
        add_submenu_page(
            'fcm',
            __('Dashboard', 'football-club-manager'),
            __('Dashboard', 'football-club-manager'),
            'edit_posts',
            'fcm',
            'fcm_page_dashboard',
            1
        );
        add_submenu_page(
            'fcm',
            __('Teams', 'football-club-manager'),
            __('Teams', 'football-club-manager'),
            'edit_posts',
            'edit.php?post_type=fcm_team',
            false,
            2
        );
        add_submenu_page(
            'fcm',
            __('Players', 'football-club-manager'),
            __('Players', 'football-club-manager'),
            'administrator',
            'edit.php?post_type=fcm_player',
            false,
            3
        );
        add_submenu_page(
            'fcm',
            __('Matches', 'football-club-manager'),
            __('Matches', 'football-club-manager'),
            'administrator',
            'edit.php?post_type=fcm_match',
            false,
            4
        );
    }
}

// Highlight submenu for child pages
if (! function_exists('fcm_select_submenu')) {
    function fcm_select_parent_menu($file)
    {
        global $submenu_file;
        if (str_starts_with($file, 'edit.php?post_type=fcm_')) {
            $submenu_file = $file;
            $file = 'fcm';
        }
        return $file;
    }

    add_filter('parent_file', 'fcm_select_parent_menu');
}

// Disable quick edit button
if (! function_exists('fcm_disable_quick_edit')) {
    function fcm_disable_quick_edit($actions)
    {
        if (str_starts_with(get_post_type(), 'fcm_')) {
            unset($actions['inline hide-if-no-js']);
        }
        return $actions;
    }

    add_filter('post_row_actions', 'fcm_disable_quick_edit');
}

// On init
if (! function_exists('fcm_init')) {
    function fcm_init()
    {
        fcm_register_team_post_type();
        fcm_register_player_post_type();
        fcm_register_match_post_type();
    }

    add_action('init', 'fcm_init');
}

// On admin menu
if (! function_exists('fcm_admin_menu')) {
    function fcm_admin_menu()
    {
        fcm_register_administration_menu();
    }

    add_action('admin_menu', 'fcm_admin_menu', 10);
}

// On activation
if (! function_exists('fcm_activated')) {
    function fcm_activated()
    {
        fcm_init();
        flush_rewrite_rules();
    }

    register_activation_hook(__FILE__, 'fcm_activated');
}

// On deactivation
if (! function_exists('fcm_deactivated')) {
    function fcm_deactivated()
    {
        // Unregister post types before refreshing rewrite rules
        fcm_unregister_team_post_type();
        fcm_unregister_player_post_type();
        fcm_unregister_match_post_type();

        flush_rewrite_rules();
    }

    register_deactivation_hook(__FILE__, 'fcm_deactivated');
}
